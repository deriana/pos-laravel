...
@section('content')
    ...
    {{-- data-grand-total harus diisi untuk JS --}}
    <div class="container mt-4" id="payment-container" data-grand-total="{{ $account->amount_due - $account->amount_paid }}">
        <div class="row">
            <!-- Left: Hutang Details -->
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <span>Status: <strong>{{ ucfirst($account->status) }}</strong></span>
                    </div>
                    <div class="card-body">
                        <p><strong>Jumlah Hutang:</strong> Rp{{ number_format($account->amount_due, 2) }}</p>
                        <p><strong>Sudah Dibayar:</strong> Rp{{ number_format($account->amount_paid, 2) }}</p>
                        <p><strong>Sisa Hutang:</strong>
                            Rp{{ number_format($account->amount_due - $account->amount_paid, 2) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-md-6">
                ...
                <form id="payment-form" action="{{ route('purchase.pay.debt', $account->id) }}"
                    method="POST">
                    @csrf
